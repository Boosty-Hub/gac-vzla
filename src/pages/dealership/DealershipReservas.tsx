...
import { CalendarDays, Plus, Search, CheckCircle, Car, User, AlertCircle } from 'lucide-react';
...
const DealershipReservas = () => {
  ...
  useEffect(() => { if (selectedDealership) fetchReservations(); }, [selectedDealership]);
                  ...
                  <TableRow key={r.id} className="[&>td]:py-1.5">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        {r.status === 'pendiente' && <Button size="sm" variant="outline" className="text-[10px] h-6 px-2" onClick={() => updateStatus(r.id, 'confirmada')}>Confirmar</Button>}
                        {r.status === 'confirmada' && <Button size="sm" variant="outline" className="text-[10px] h-6 px-2" onClick={() => updateStatus(r.id, 'en_proceso')}>Iniciar</Button>}
                        {r.status === 'en_proceso' && <Button size="sm" variant="outline" className="text-[10px] h-6 px-2" onClick={() => updateStatus(r.id, 'completada')}>Completar</Button>}
                        {(r.status === 'pendiente' || r.status === 'confirmada') && <Button size="sm" variant="ghost" className="text-[10px] h-6 px-2 text-destructive" onClick={() => updateStatus(r.id, 'cancelada')}>Cancelar</Button>}
                      </div>
                    </TableCell>
                  </TableRow>
                );
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label className="text-xs">Servicio *</Label><Select value={fService} onValueChange={setFService}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Seleccionar" /></SelectTrigger><SelectContent>{SERVICE_TYPES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
