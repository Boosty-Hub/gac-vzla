...
const DealershipReservas = () => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    // `.select('id')` para distinguir "RLS lo rechazo en silencio" de "se guardo": sin esto,
    // un asesor sin vinculo al concesionario de la cita actualiza 0 filas y de todos modos ve
    // "Estado actualizado", aunque la cita se haya quedado en el estado anterior (ver
    // handleComplete/handleCancelConfirm, que ya usan este mismo patrón).
    const { data: updated, error } = await supabase
      .from('reservations')
      .update({ status: newStatus })
      .eq('id', id)
      .select('id');
    if (error) { toast.error('Error al actualizar estado'); console.error(error); return; }
    if (!updated || updated.length === 0) {
      toast.error('No tenés permiso para actualizar esta reserva: verificá que estés vinculado al concesionario correcto.');
      return;
    }
  ...
  const openDetail = async (r: Reservation) => {
    ...
    if (r.vehicle_id) {
      ...
      const { data: history, error: historyError } = await (supabase.rpc as any)('vehicle_service_history', {
        p_vehicle_id: r.vehicle_id,
      });
      if (historyError) console.error('Error cargando el historial del vehículo:', historyError);
      setVehicleHistory(((history as VehicleHistoryRow[]) || [])
        .filter(h => h.id !== r.id)
        .map(h => ({
          id: h.id,
          ...
          status: h.status,
          // `vehicle_service_history` sólo devuelve servicio, centro y encuesta — no expone
          // `notes`, `internal_notes` ni `technical_report_url`. Ver comentario en la migración.
          notes: null,
          internal_notes: null,
          service_notes: h.service_notes,
          cancellation_reason: null,
          technical_report_url: null,
  ...
  const handleComplete = async () => {
    ...
    if (error) { toast.error('Error al completar'); console.error(error); }
      ...
      // La encuesta va DESPUÉS de que la cita quedó cerrada y sólo si la pidieron. Sin await:
      // si Kommo tarda o falla, el servicio ya está completado igual.
